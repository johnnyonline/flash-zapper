...
# USDaf/sfrxUSD Curve StableNG Pool
USDAF_INDEX_USDAF_SFRXUSD_CURVE_POOL: constant(uint256) = 0
SFRXUSD_INDEX_USDAF_SFRXUSD_CURVE_POOL: constant(uint256) = 1
USDAF_SFRXUSD_CURVE_POOL: immutable(address)
...
@deploy
def __init__(owner: address, curve_pool: address):
    """
    ...
    USDAF_SFRXUSD_CURVE_POOL = curve_pool

    self._max_approve(USDAF, USDAF_SFRXUSD_CURVE_POOL)
    ...
    self._max_approve(IERC20(SFRXUSD.address), USDAF_SFRXUSD_CURVE_POOL)
...
def _swap_from(amount: uint256, min_amount: uint256) -> uint256:
    ...
    amount_out: uint256 = curve_stableswap.swap(
        USDAF_INDEX_USDAF_SFRXUSD_CURVE_POOL,
        SFRXUSD_INDEX_USDAF_SFRXUSD_CURVE_POOL,
        amount,
        USDAF_SFRXUSD_CURVE_POOL,
        msg.sender,
    )
...
def _swap_to(amount: uint256, min_amount: uint256) -> uint256:
    ...
    """
    # Pull sfrxUSD
    extcall IERC20(SFRXUSD.address).transferFrom(msg.sender, self, amount, default_return_value=True)
