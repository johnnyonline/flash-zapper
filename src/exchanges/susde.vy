...
@notice Swaps sUSDe for USDaf and vice versa
"""
...
# scrvUSD/sUSDe Curve StableNG Pool
SCRVUSD_INDEX_SCRVUSD_SUSDE_CURVE_POOL: constant(uint256) = 0
SUSDE_INDEX_SCRVUSD_SUSDE_CURVE_POOL: constant(uint256) = 1
SCRVUSD_SUSDE_CURVE_POOL: constant(address) = 0xd29f8980852c2c76fC3f6E96a7Aa06E0BedCC1B1
...
SUSDE: constant(IERC20) = IERC20(0x9D39A5DE30e57443BfF2A8307A4256c8797A3497)
...
def __init__(owner: address):
    ...
    self._max_approve(IERC20(SCRVUSD.address), SCRVUSD_SUSDE_CURVE_POOL)
    self._max_approve(SUSDE, SCRVUSD_SUSDE_CURVE_POOL)
...
def _swap_from(amount: uint256, min_amount: uint256) -> uint256:
    ...
    # crvUSD --> scrvUSD
    amount_out = extcall SCRVUSD.deposit(amount_out, self)

    # scrvUSD --> sUSDe
    amount_out = curve_stableswap.swap(
        SCRVUSD_INDEX_SCRVUSD_SUSDE_CURVE_POOL,
        SUSDE_INDEX_SCRVUSD_SUSDE_CURVE_POOL,
        amount_out,
        SCRVUSD_SUSDE_CURVE_POOL,
        msg.sender,
    )
...
def _swap_to(amount: uint256, min_amount: uint256) -> uint256:
    ...
    """
    # Pull sUSDe
    extcall SUSDE.transferFrom(msg.sender, self, amount, default_return_value=True)
    ...
    # scrvUSD --> crvUSD
    amount_out = extcall SCRVUSD.redeem(amount_out, self, self)
